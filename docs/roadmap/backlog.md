...
| DEBT-18 | El alta de jugador vive en el game server, no en Next.js | `POST /api/auth/register` y `POST /api/auth/login` los sirve `internal/httpapi` del game server, con bcrypt, y es él quien emite el game ticket. La arquitectura objetivo de [../decisions/ADR-010-authentication-game-ticket.md](../decisions/ADR-010-authentication-game-ticket.md) sitúa esos endpoints en una API route de Next.js. **Al intentar cerrarla se descubrió que, tal como está enunciada, es irrealizable:** [migrations.md](../database/migrations.md) establece como *regla de arquitectura* que `apps/web` no tendrá credenciales de PostgreSQL ni importará ningún cliente SQL, y [frontend.md](../architecture/frontend.md) que Next.js sólo toca `EO_AUTH_JWT_SECRET`. Si Next.js no puede leer la base, no puede verificar una contraseña — y bcrypt tiene que vivir donde vive `password_hash`. Mover la autenticación exige antes decidir el **mecanismo de sesión web**, que [system-context.md](../architecture/system-context.md) marca **TBD (fuera de MVP)**. | El proceso autoritativo de simulación carga con manejo de contraseñas y con el coste de bcrypt, que es deliberadamente lento: superficie de ataque y latencia en el proceso que menos debería tener ninguna de las dos. **Mitigado, no resuelto**: los dos endpoints tienen ya límite de tasa por dirección de origen (`EO_AUTH_RATE_LIMIT_*`), que era lo que convertía la deuda en un vector explotable — bcrypt sin autenticación previa y sin límite es un amplificador de denegación de servicio contra el proceso del game loop. | Cuando se decida el mecanismo de sesión web. Las dos salidas coherentes son: (a) que Next.js tenga su propio almacén de credenciales y llame a un endpoint interno del game server para el alta del mundo, o (b) relajar la regla de «Next.js sin base de datos» mediante un ADR que la supersede. Ninguna se toma sin decisión explícita: la primera introduce un segundo sistema de identidad, la segunda contradice una regla que hoy es dura. |
