...
| DEBT-19 | Tests de integración escritos pero no ejecutados | La suite de integración contra PostgreSQL y Redis reales existe y está *gated* por `EO_INTEGRATION=1`, pero **no se ha ejecutado nunca**: el daemon de Docker Desktop no arrancó en la máquina de desarrollo. La suite unitaria, de contrato, de simulación y de recuperación sí corre y está en verde. | Todo lo que sólo verifica la integración —migraciones y sus `.down.sql`, atomicidad real del bootstrap, `CHECK`s y triggers de PostgreSQL, expiración real de claves de Redis, el índice único parcial de `unit_movements`— está **sin verificar**. La documentación no debe contarlo como aprobado. | Inmediata: arrancar el daemon de Docker, o montar la CI con Postgres y Redis como *services*. Es la deuda con el disparador más barato de todas y la primera que debe cerrarse, porque hasta entonces ningún milestone puede declararse cerrado. |
